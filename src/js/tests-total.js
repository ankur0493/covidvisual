...
.then(function(data){
  ...
  var y = d3.scaleLinear()
      .domain([0,d3.max(data, d => d.samples_tested)]).nice()
  ...
  function hover(svg, path) {
    ...
    function moved() {
      ...
      const s = data[i];
      tooltip.attr("transform", `translate(${x(s.date)},${y(s.samples_tested)})`);
  ...
  var line = d3.line()
    .defined(d => !isNaN(d.samples_tested))
    .x(d => x(d.date))
    .y(d => y(d.samples_tested));
